...
pub fn rewrite_authorship_after_squash_or_rebase(
    ...
    if changed_files.is_empty() {
        if commits_have_authorship_notes(repo, &source_commits)? {
            debug_log(
                "No AI-touched files in merge, but notes exist in source commits; writing metadata-only authorship log with contributors",
            );
            ...
            if !contributors.is_empty() {
                debug_log("No AI notes but building contributors for manual contributions");
